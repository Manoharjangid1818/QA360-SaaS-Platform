...
export default function DashboardPage() {
  ...
  return (
    <div>
      <Header title="Dashboard" subtitle="Overview of your QA testing activity" />
      <div className="p-6 space-y-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
          <div className="xl:col-span-2"><StatCard title="Total Test Cases" value={stats.totalTestCases} icon={ClipboardList} color="blue" subtitle="All test cases" /></div>
          <div className="xl:col-span-2"><StatCard title="Passed" value={stats.passed} icon={CheckCircle} color="green" subtitle="Tests passing" /></div>
          <div className="xl:col-span-2"><StatCard title="Failed" value={stats.failed} icon={XCircle} color="red" subtitle="Need attention" /></div>
          <div className="xl:col-span-2"><StatCard title="Pending" value={stats.pending} icon={Clock} color="yellow" subtitle="Not yet run" /></div>
          <div className="xl:col-span-2"><StatCard title="Total Bugs" value={stats.totalBugs} icon={Bug} color="purple" subtitle="Tracked issues" /></div>
          <div className="xl:col-span-2"><StatCard title="Open Bugs" value={stats.openBugs} icon={AlertCircle} color="red" subtitle="Require action" /></div>
        </div>
        ...
        <div className="card p-6">
          ...
          <div className="space-y-3">
            {stats.recentActivity.map((item) => {
              ...
              const actionVerbs: Record<string, string> = { created: 'Created', updated: 'Updated', resolved: 'Resolved' };
              return (
                <div key={item.id} className="flex items-center gap-4 py-2 border-b border-gray-100 last:border-0">
                  <span className={`badge ${typeColors[item.type]} capitalize`}>{item.type.replace('_', ' ')}</span>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm text-gray-900 truncate"><span className="font-medium">{actionVerbs[item.action]}:</span> {item.title}</p>
                  </div>
