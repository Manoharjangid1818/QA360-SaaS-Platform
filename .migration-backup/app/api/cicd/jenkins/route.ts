// GET /api/cicd/jenkins — Jenkins-specific builds + stats
...
export async function GET() {
  ensureInitialized();
  const pipelines = getPipelinesByProvider('jenkins');
  ...
  return NextResponse.json({
    ...
    stats: {
      ...
      buildTrend: allStats.buildTrend.map((d) => ({ date: d.date, value: d.jenkins })),
    },
  });
}
